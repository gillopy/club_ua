      ...
      <div className="container px-4 md:px-6">
        <div className="flex flex-col items-center justify-center space-y-4 text-center">
          <div className="space-y-2">
            ...
            <p className="max-w-[900px] text-space-300 md:text-lg">{aboutUs.description}</p>
          </div>
        </div>
        <div className="mx-auto mt-12 grid max-w-5xl gap-8 lg:grid-cols-2">
          <div className="flex flex-col justify-center space-y-6">
            <div>
              <h3 className="text-xl font-medium text-white mb-2 tracking-tighter">Our History</h3>
              <p className="text-space-200">{aboutUs.history}</p>
            </div>
            <div>
              <h3 className="text-xl font-medium text-white mb-2 tracking-tighter">Key Achievements</h3>
              <ul className="grid gap-3">
                {aboutUs.achievements.map((achievement, index) => (
                  <li key={index} className="flex items-start gap-2">
                    <div className="mt-1 cosmic-glow">
                      <Check className="h-5 w-5 shrink-0 text-cosmic-400" />
                    </div>
                    <span className="text-space-200">{achievement}</span>
                  </li>
                ))}
              </ul>
            </div>
